...
console.log('>>> DEBUG: Loading the most recent version of marketing-ai-specialists.js...');
...
// In logic/marketing-ai-specialists.js
// DELETE your entire existing calculateScores function and REPLACE it with this block.
...
const calculateScores = (answers, answerKey) => {
    ...
    const capabilityMap = {
        ...
        'Digital & Global Fluency': ['digitalAndDataKnowledge', 'globalAndCulturalKnowledge', 'futureAndThoughtLeadership', 'channelsAndDistributionApplication']
    };
    
    console.log('>>> DEBUG: capabilityMap successfully created inside calculateScores.'); // <-- ADD THIS LINE

    const coreCapabilityScores = {};
    const benchmarkData = {
        'Strategic Acumen': { industryAverage: 65, topPerformer: 92 },
        'Commercial Rigor': { industryAverage: 70, topPerformer: 94 },
        'Brand Craft': { industryAverage: 68, topPerformer: 89 },
        'Execution & Leadership': { industryAverage: 72, topPerformer: 91 },
        'Digital & Global Fluency': { industryAverage: 60, topPerformer: 88 }
    };

    for (const [capability, dimensions] of Object.entries(capabilityMap)) {
    const total = dimensions.reduce((sum, key) => sum + (dimensionScores[key] || 0), 0);
    const userScore = total / dimensions.length;
    
    coreCapabilityScores[capability] = {
        yourScore: parseFloat(userScore.toFixed(1)),
        industryAverage: benchmarkData[capability].industryAverage,
        topPerformer: benchmarkData[capability].topPerformer
    };
    }
